                            ...
                            @if ($data['postCatalogues'])
                                ...
                                <tr id="row_user_{{ $language->id }}">
                                    ...
                                    <td>
                                        ...
                                        <a href="javascript:void(0)" id="deleteItem_{{ $language->id }}" onclick="deleteItem({{ $language->id }})" data-action="false" data-title="Bạn muốn khôi phục ngôn ngữ : {{ $language->name }}" data-message="khôi phục ngôn ngữ bạn có thể sử dụng nó !" data-url="{{ route('admin.post.delete', $language->id) }}"><button type="button" class="btn btn-primary btn-circle"><i class=" ti-back-right"></i> </button></a>
                                        <a href="javascript:void(0)" id="deleteItem_{{ $language->id }}" onclick="deleteItem({{ $language->id }})" data-action="false" data-title="Bạn muốn xóa post: {{ $language->name }}" data-message="Xóa post này có thể ảnh hưởng đến dữ liệu !" data-url="{{ route('admin.post.delete', $language->id) }}"><button type="button" class="btn btn-danger btn-circle"><i class="ti-trash"></i> </button></a>
                                    </td>
                                </tr>
                                @endforeach
                                <tr>
                                    <td colspan="7">
                                        {{ $data['postCatalogues']->withPath('post')->onEachSide(1)->links('vendor.pagination.custom') }} 
                                    </td>
                                </tr>
                                @endif
                        </tbody>
