<div class="card">
    <div class="card-header">
        <h5>Album Ảnh <div class="upload-album pull-right"><a href="javascript:void(0)" class="upload-picture">Chọn Hình</a></div></h5>
        
    </div>
    @php
        $gallery = (isset($album) && count($album)) ? old('album',$album) : old('album');
    @endphp
    <div class="card-body">
        <div class="click-to-upload @if(isset($album) && count($album)) hidden-xl-down @endif">
            <div class="icon">
            ...
            {{-- <input type="hidden" value="{{ old('image',$data['post']->image ?? null) }}" name="image" class="form-control upload-image-avartar"> --}}
            <div class="card-title">Sử dụng nút chọn hình ảnh hoặc click vào đây để thêm hình ảnh</div>
        </div>
        <div class="upload-list @if(empty($album)) hidden-xl-down @endif">
            <ul id="sortable" class="clearfix data-album sortui ui-sortable">
                @if (isset($gallery) && count($gallery))
                    @foreach ($gallery as $albu)
                    <li class="ui-state-default">
                        <div class="thumb">
                            <span class="image span img-scaledown">
                                <img src="{{ $albu }}" alt="{{ $albu }}">
                                <input type="hidden" name="album[]" value="{{ $albu }}">
                            </span>
